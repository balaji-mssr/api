...
public class RiverBusStatusServiceImpl implements StatusUpdateService {
	@Override
	public List<Line> getLineStatusListFromAPI() {
		String response = ResponseUtils.getBetaLineResponseByModes(Modes.RIVER);
	...
	public List<Line> getLiveLineStatusListFromICS() {
		String XML = get(AppProperties.LIVE_RIVER_ICS_FEED_END_POINT).andReturn().asString();
	...
	public List<Line> getWeekendLineStatusFromAPI(Boolean isStub) {
		String rootResponse = ResponseUtils.getWeekendBetaLineResponseByModes(Modes.RIVER);
	...
	public List<Line> getWeekendLiveLineStatusFromICS(Boolean isStub) {
		String XML = get(AppProperties.LIVE_RIVER_ICS_WEEKEND_FEED_END_POINT).andReturn().asString();
		List<Line> lineICSDto = XMLHelper.getICSStatusDTO(XML);
		return lineICSDto;
	}
